...
**Symphact: aktor-szubsztrátum a post-Dennard korszakra — egyetlen alapegység klasszikus számításra, AI-ágenses feladatokra és hardvervezérlésre**
...
- Online outreach + dokumentáció (videó-production, GitHub Pages hosting, technikai dokumentáció kibővítése — **utazás-mentes**): €2,500
...
| **Anthropic Agent SDK / OpenAI Agents / LangGraph** | Multi-agent koordinációs framework Python-ban | Function-calling + JSON + JIT-eval; nincs capability-szintű izoláció; audit eset-eset alapú | Capability-vel korlátozott aktorok, runtime + HW-szintű validáció, supervision-alapú audit, EU AI Act-konform alapok |
...
1. **M0.3 + M0.4 + M0.5 első szeletei KÉSZ** — supervision (let-it-crash, OneForOne / AllForOne), scheduler (`TInlineScheduler` + `TDedicatedThreadScheduler`), és a BCL-only `TInMemoryJournal` + `TInMemorySnapshotStore` leszállítva, 186 zöld teszt
2. **Symphact README frissítése** — aktuális státusz (M0.1–M0.4 ✅, M0.5 BCL-only szeletek ✅, M0.5 content-addressed backend folyamatban), 186 teszt, roadmap update
...
|--------|-------|-------------|
| 1.3 | 2026-05-16 | Paradigma-átírás: post-Dennard szubsztrátum + egyesítő tézis (`TActor` mint OS-folyamat / hardvermeghajtó / AI-ágens / LIF-neuron). Tesztszám 142 → 186; `TCasJournal`; új „Nyitott kutatási kérdések" (9 pont) + „Hosszú távú vízió" szekciók; kohorta-vállalás → M5 (realisztikusra szűkítve). Ördögügyvédi tényellenőrzés: Apple-analógia → Transputer/Lisp Machine; Dennard-évszám forrásolva (Bohr 2007); „dark silicon" hivatkozás pontosítva; Armstrong-idézet lágyítva; survivorship bias kiegyenlítve; audit + konferencia kivéve (induló projekt / kapacitás); CLI-CPU beadási dátum javítva (2026-04-14); Atlasz 26 év éles üzem. |
| 1.1 | 2026-05-01 | Beadás-kész a 13. nyílt körre; új M1–M6 mérföldkő-struktúra; €30 000 költségvetés; CLI-CPU #5, #9 beépítve. |
| 1.0 | 2026-04-23 | Kezdeti draft. M0.3–M3.2 + CFPU demó + biztonsági audit; €30 000 / 12 hónap; CLI-CPU pályázattól scope-elhatárolva. |
