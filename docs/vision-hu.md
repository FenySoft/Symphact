# Symphact — a Cognitive Fabric Processing Unit (CFPU) aktor-alapú operációs rendszere

> **📦 Implementáció:** [`src/Symphact.Core`](../src/Symphact.Core) — a tényleges runtime kód, tesztek és API a `src/` és `tests/` mappákban van. Ez a dokumentum a hosszú távú **víziót** rögzíti.
...
> *A Symphact a **CFPU** hardveres platformra készül — a CFPU referencia implementációja a **CLI-CPU** projekt ([github.com/FenySoft/CLI-CPU](https://github.com/FenySoft/CLI-CPU)). A CFPU ↔ CLI-CPU kapcsolat részletei: [CLI-CPU FAQ #1](https://github.com/FenySoft/CLI-CPU/blob/main/docs/faq-hu.md#1-mi-a-cfpu-és-mi-a-kapcsolata-a-cli-cpu-val).*
...
### A Symphact mint tiszta lapról indulás

A Symphact-t **modern korban** tervezzük, modern feltételekre:
...
A Symphact tehát **nem a Linux mellérakódik**, hanem **a Linux utóda**. Ahogy az x86 leváltotta a mainframe-et, a mobile leváltotta a desktopot, a cloud leváltotta a fizikai szervert — **a Cognitive Fabric + Symphact leváltja a shared-memory + Linux kombinációt** a most formálódó AI-vezérelt, biztonság-kritikus, masszívan elosztott korszakban.
...
A Symphact-ben ez **minden szinten** érvényes, a legkisebb device driver-től a legfelsőbb alkalmazás-aktorig.
...
- Egyetlen Symphact **több chipre is elterjedhet** (F7+ elosztott)
...
Ez a CHERI és a seL4 biztonsági modellje, amit a Symphact natívan használ, **mert a hardver (CLI-CPU) eleve shared-nothing**.

### 7. Hot code loading

Egy **futó rendszerbe új CIL kódot lehet tölteni** leállás nélkül. Egy aktor fogadhatja az utolsó üzenetet a `v1` verzióval, és a következőt a `v2` verzióval — **a supervisor** koordinálja az átállást. Ez az Erlang OTP 40 éves funkciója, amit a Symphact natívan támogat.
...
A Symphact alapértelmezésben **determinisztikus**: ugyanaz a bemenet-sorozat ugyanazt az állapotot eredményezi. Ez:
...
## A Linux öröklött problémái és a Symphact válasza
...
| Probléma-kategória | Linux (Unix-örökség) | Symphact |
|-------------------|----------------------|-----------|
...
Legyünk őszinték: a Linux **hatalmas siker**. Évi $15+ milliárd vállalati ökoszisztéma, futtat minden modern felhőt, minden Android telefont, minden szupercsomópontot. Ezt **nem lehet figyelmen kívül hagyni**, és a Symphact-nek sem szabad azt hinnie, hogy egy éjszaka alatt leváltjuk.
...
A Symphact **minden ilyenre** törekszik, de **más alapokon**.

### Mi lesz a Linux-szal?

A Symphact **nem akarja, hogy a Linux egyik napról a másikra eltűnjön**. A valós átmenet hosszú távú, talán **10-20 év**:

| Időszak | Linux pozíciója | Symphact pozíciója |
|---------|-----------------|---------------------|
...
Ezek a Symphact „kernelje", de nem kernel módban, hanem **néhány Rich core-on futó speciális aktor**:
...
Az Erlang OTP négyféle restart stratégiát definiál: `one_for_one`, `one_for_all`, `rest_for_one`, `simple_one_for_one`. A Symphact ugyanezeket támogatja.
...
3. **Inter-chip** (elosztott, másik Symphact node-on) — ~100–1000 ciklus, egy dedikált network aktoron keresztül
...
Ez a shared-nothing modell **egyik legnagyobb egyszerűsítése**. Az Akka.NET-ben a globális .NET GC a szűk keresztmetszet magas terhelésen; a Symphact-en **ez a probléma nem létezik**.
...
Egy **capability** egy nem-hamisítható token, amely egy konkrét jogosultságot reprezentál. A Symphact-ben **az aktor referencia maga egy capability**:
...
**Ez egy hardveresen megvalósított capability-based OS** — amit eddig csak a seL4 és a CHERI kínált akadémiai szinten, és amit a Symphact a .NET világba hoz.

## Dinamikus kód betöltés

### Hot code loading

Egy futó Symphact-be **új CIL kódot** lehet betölteni leállás nélkül. A folyamat:
...
Minden hardveres periféria egy **device aktor** a Symphact-ben. A device aktor:
...
A Symphact **nem egy külön termék** a CLI-CPU hardverhez képest — **hanem az a szoftveres réteg, ami a Cognitive Fabric pozicionálást valódivá teszi**. Ugyanaz a hardver (CLI-CPU multi-core + mailbox + heterogén Nano+Rich) **különböző** használati módokat támogat **ugyanazzal** a Symphact alapján:
...
**Ugyanaz az operációs rendszer**, **ugyanaz a hardver**, **ugyanaz a programozási modell** — **más** application aktorok, **más** szerepek. A Symphact a „lingua franca", ami minden Cognitive Fabric alkalmazást egységes platformra tesz.

## Fázisos megvalósítás

A Symphact **nem egy nagy ugrás**, hanem **organikusan épül fel** az F1–F7 fázisok során, minden fázisban hozzáadva a következő réteget.
...
**Ez az első valódi hardveres „Symphact"**, bár még csak egy-aktoros.
...
Ez **az első felhasználható Symphact**, amely képes valós C# programokat futtatni aktor-orientált módon.
...
Ez az **FPGA-verifikált, elosztott** Symphact — a valódi multi-chip Cognitive Fabric első demonstrációja.
...
Ez a **szilíciumra érett** Symphact.
...
- NuGet csomagok (`Symphact.Core`, `Symphact.Actor`, `Symphact.Devices`)
...
**Ez a Symphact kijutása a kutatási szintről a valós fejlesztői platform szintjére.**
...
Ezek nem ígéretek, hanem **architekturális motiváció**: a CFPU + Symphact tervezési döntéseinek igazolása, hogy az aktor-alapú, shared-nothing modell nem csupán elméleti elegancia, hanem a jövő számítási problémáinak természetes nyelve.
...
## Amit a Symphact nem **utánoz** — tudatos architektúra-döntések
...
Helyettük a Symphact **modern alternatívákat** ad: `Spawn<Actor>()` (nem `fork`), `Send(actorRef, msg)` (nem `write()` fd-re), `Receive()` (nem `read()` blokkolva), `ActorRef` (nem `fd`). Ha egy régi POSIX alkalmazás fordítására van szükség, **egy szoftveres kompatibilitási réteg** építhető a Symphact-re (mint a Windows WSL2 fordítva) — de **a natív programozási modell egyértelműen modernebb és biztonságosabb**.

### 2. Nem monolit kernel — helyette aktor-hierarchia

Nincs kernel tér, nincs user tér, nincs system call overhead. **Miért jobb:** a kernel/user mode switch minden rendszerhívásnál ~1000 ciklus veszteséget okoz, és a Spectre/Meltdown/L1TF mind a privilégium-határokat próbálják átlépni. A Symphact-en **nincsenek ilyen határok** — minden komponens aktor, a hardveres shared-nothing isolation garantálja azt, amit más OS-ek a kernel/user mode switch-csel biztosítanak. Egy aktor nem tud egy másik aktor memóriájába írni **nem azért, mert a kernel megállítja**, hanem mert **fizikailag nem létezik olyan útvonal**.

### 3. Nem globális fájlrendszer sémát — helyette strukturált storage service

A fájl **nem egy bájt-stream** a Symphact-ben. **Miért jobb:** a Unix „minden fájl" absztrakció elfed sok modern adatstruktúrát (idősoros adat, gráf, objektum-séma, eventual-consistent store). Ezek ma **mind a fájlrendszer fölött** vannak rárakott rétegként (SQLite, RocksDB, LevelDB), ami komplexitást és sebezhetőséget hoz.

A Symphact-en az **adat aktor-üzenetek** formájában érkezik és megy, és a „storage service" egy **strukturált aktor-rendszer**, ami közvetlenül ismeri az adatstruktúrákat. Egy POSIX-kompatibilitási réteg a szélén megadhatja a hagyományos fájlrendszer API-t, ha szükséges.
...
A Symphact **capability-based security** modellje **árnyaltabb, finomabb, és delegálható**. Egy aktornak **csak akkor** van jogosultsága egy művelethez, ha valaki **átadta** neki a megfelelő capability-t. Nincs globális „root" — mindenki csak azt teheti, amihez kifejezetten kapott felhatalmazást. Ez a CHERI és seL4 modellje, amit évtizedes kutatás bizonyított.
...
A Symphact-en **alapértelmezésben** type-safe, hardveres GC-vel, és a CIL ECMA-335 verifikálható kód szemantika építve. Egy fejlesztő **nem tud** memory corruption bugot írni a Symphact-re, mert sem a nyelv (C#), sem a runtime (CLI-CPU), sem az ISA (CIL-T0/Rich) **nem engedi**.
...
A Symphact-en egy driver **egy aktor**, és ha hibázik, a **supervisor újraindítja**. A többi rendszer **érintetlen marad**. Ez **40 év alatt** bizonyított megközelítés az Erlang-ban (Ericsson AXD301 9-nines availability), és a Symphact természetesen ezt használja.

## Távlati lehetőségek — amit a Symphact **természetesen** kinyit

Ez a szekció azokat a területeket írja le, amelyek **nem az első generáció céljai**, de ahol a Symphact architektúrája **eredendően** alkalmas, és **hosszú távon** (F7 után, 2035+ időszak) valósággá válhat — sőt, bizonyos területeken **drámaian jobb** lehet, mint a mostani Linux/Windows/macOS megoldások. Ezek nem „majd valamikor" mellékes gondolatok, hanem **a projekt jövőbeli lehetőség-horizontja**.

### 1. Interaktív asztali UI — natívan aktor-alapú

A Symphact-en **egy asztali UI természetesen illeszkedik**, mert minden UI elem alapvetően aktor-szerű:

| UI komponens | Hagyományos OS-en | Symphact-en |
|-------------|-------------------|--------------|
...
**Egy Symphact Desktop** nem a X11/Wayland modellt másolná, hanem **natívan aktor-alapú**: minden widget egy `UiWidgetActor`, minden ablak egy `WindowActor`, a compositing egy `RenderSupervisorActor`. Egy **React/Flutter-szerű API** C#-ban, közvetlenül a Symphact runtime-jára építve. Ez **sokkal egyszerűbb** mint a Linux stack, és **sokkal robusztusabb**.

**Mikor:** 2035+ táv, F7 után egy külön „Symphact Desktop" projektben. Nem az első generáció, de **nem is elérhetetlen** — csak **nincs még idő** hozzá. Amikor eljön a pillanat, a rendszer **készen áll rá**.
...
| Játék komponens | Hagyományos megvalósítás | Symphact-en |
|----------------|--------------------------|--------------|
...
- **Formally verifiable game logic** — egy kompetitív játék (esport) logikája **matematikailag bizonyíthatóan fair**, ha a Symphact-en fut — nincsenek anti-cheat heurisztikák, a rendszer **architekturálisan** nem engedi a cheating-et
...
**Unity DOTS** és **Unreal Mass** szoftveresen próbálják ugyanazt elérni a hagyományos CPU-n, amit a Symphact **hardveresen** adna ingyen.

**Mikor:** 2030+ időszak, ha egy játékstúdió vagy egy indie csapat **partnerként** kezdi használni. Egy **realtime engine** fejlesztése több év, de **architekturálisan** a Symphact az **ideális** alapja egy következő generációs játékmotornak.
...
**Minden rétegben van overhead, van sebezhetőség, van komplexitás**. A Symphact ezt **drasztikusan** egyszerűsíti, mert **maga a rendszer aktor-orientált, ami természetesen illeszkedik az AI-hoz**.

#### Hét AI-domain, ahol a Symphact fundamentálisan jobb
...
A mai **LLM agent** rendszerek (AutoGen, Claude Agent, OpenAI Swarm) **szoftveres layer**-ek a hagyományos OS fölött, Python-ban. Egy Symphact-en **minden agent egy saját hardveres aktor**, Rich core-on futó teljes LLM-mel, vagy Nano core-on futó kis specialista modellel.
...
Ez **forradalmian fontos** a **regulated AI** jövőjében. Az EU AI Act és hasonló szabályozások pontosan **ezt** fogják követelni — és a Symphact az **egyetlen platform**, ami ezt **architekturálisan** meg tudja adni.
...
A Symphact **architekturális** védelmet ad:
...
| AI alkalmazás | Mit ad a Symphact |
|--------------|---------------------|
...
Mert eddig az AI platform a **számítási teljesítmény** versenyében volt. „Több FLOPS, több paraméter, nagyobb modell." A Cognitive Fabric + Symphact **más tengelyen** verseng: **programozhatóság, biztonság, skálázódás, tanúsíthatóság**.
...
**Ha a projekt ezt eléri, akkor a Symphact nem csak a Linux utódja lesz, hanem az AI kor első natív operációs rendszere.** Ez a CLI-CPU projekt **legtávolabbi, legambiciózusabb** horizontja.

### 4. Ökoszisztéma és platform — hosszú táv

A Linux 30 év alatt hatalmas szoftver-ökoszisztémát épített fel (`apt`, `dnf`, `pacman`, `npm`, `PyPI`, `crates.io`, stb.). A Symphact **nem fogja ezt egy napról a másikra lemásolni**, mert nem akarja — a **natív .NET ökoszisztéma** (NuGet) + a Symphact-specifikus csomagok **évek alatt** épülnek fel, egy **más** paradigmára.

**Ami viszont szükségszerű:** minden meglévő szoftver, ami `dotnet publish`-sel lefordítható és egy NuGet csomag, **hosszú távon** natívan futhat a Symphact-en. A .NET ökoszisztéma már most is **~400 000+ csomag** a NuGet-en, aminek egy jelentős része **nem igényel P/Invoke-ot** és **nem igényel reflection-t**, tehát **natívan** fut a CLI-CPU + Symphact kombináción.
...
Az egyetlen terület, ahol a Symphact **explicit nem akar jelen lenni**: a **legacy POSIX binárisok natív futtatása**. Egy meglévő C/C++ Linux program **nem fog natívan** futni Symphact-en. Ha valaki kétségbeesetten akarja, egy **kompatibilitási réteg** (Linux Subsystem for Symphact, LSNOS) épülhet a tetejére — de ez **nem az alapvető modell**, és **nem ajánljuk** új kód fejlesztésére.
...
A Symphact fejlesztése **nem önálló fázis** a roadmap-ben, hanem **az F1-F7 fázisok mentén organikusan** épül ki. Az első konkrét lépések:

1. **F1-ben**: a C# referencia szimulátornak legyen egy minimális `Symphact.Core` könyvtár-projekt, amely az aktor absztrakciókat adja (`Actor<T>`, `Spawn`, `Send`, `Receive`). **Ez nem OS még**, csak egy fejlesztői kényelem, de már most aktor-orientált kódot lehet írni.

2. **F3 Tiny Tapeout bring-up programnál**: az echo neuron demó legyen egy **valós Symphact aktor**, nem csak egy C# program. Az aktor interfész minimum is, de **explicit** aktor-ként van megírva.

3. **F4 multi-core FPGA demónál**: az első **valódi** Symphact alfa — 4 aktor, scheduler, router, minimal supervisor. Ez a *project első mérföldköve*, ahol „OS"-ről beszélhetünk.
...
1. **Biztonsági nyomás** — az AI-vezérelt kód-generálás, a supply chain támadások, és a Spectre-utódok miatt a Linux architekturálisan nem tud lépést tartani. A Symphact-en **ezek a támadási osztályok architekturálisan kizártak**.
...
3. **Distributed-first világ** — a cloud, az edge, az IoT, az AI agent-ek mind **elosztott** rendszerek alapértelmezésben. A Linux a „lokális gép + networking rárakva" modellt örökölte. A Symphact **natívan elosztott**.

4. **Fault tolerance elvárás** — a 9-nines elérhetőség, a „never reboot" elvárás, a kritikus alkalmazások mind **supervision-t** kérnek, amit a Linux csak nehezen és rárakott rétegekkel tud (systemd + K8s + service mesh). A Symphact-en ez **natív**.

5. **Formális verifikáció szükséglete** — a safety-critical rendszerek (medical, automotive, aviation, kritikus infrastruktúra) egyre szigorúbb tanúsítást követelnek, amit egy **40 M sor C kernel** soha nem fog elérni. A Symphact **formálisan verifikálható**.

6. **AI paradigma-váltás** — az AI korszakban az operációs rendszer **nem egy passzív kiszolgáló**, hanem egy **aktív részvevő**. Az agent-alapú AI, a federated learning, a formálisan verifikált AI, a prompt injection védelem — mind **architekturális** igények, amiket a Linux rárakott rétegekkel nem tud adni, de a Symphact **natívan**.

### Mi lehet a Symphact távlati hatása

Ha minden tervezett irányban sikerül:

- **Kritikus infrastruktúra** (automotive, medical, aviation, energetika) — Symphact a tanúsított alap, 2030-tól
- **Hyperscale cognitive computing** — Symphact + CLI-CPU mint új kategóriás cloud szerver, 2035-től
...
Ha a projekt eljut F6-F7-ig, és a Symphact működőképes, akkor **a CLI-CPU + Symphact együtt** egy új számítási paradigma első fizikai megvalósítása lesz, amit sem az Erlang, sem a QNX, sem a seL4, sem a Singularity nem tudott a saját korukban megvalósítani:
...
**A CLI-CPU csak egy chip. A Symphact egy új korszak.**
