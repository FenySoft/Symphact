...
**Symphact: actor-based operating system co-designed with the CFPU**
...
**What we are asking for, briefly:** Symphact is a **capability-based actor runtime for .NET**, **Apache-2.0** licensed, co-designed with the Cognitive Fabric Processing Unit (CFPU) open-silicon project ([`FenySoft/CLI-CPU`](https://github.com/FenySoft/CLI-CPU), CERN-OHL-S-2.0). **At submission (2026-05-23): 186 passing xUnit tests** cover the actor core, supervision (let-it-crash), per-actor parallel scheduler (CFPU dedicated-core-per-actor simulation), and the BCL-only event-sourcing journal + snapshot store — strict TDD, ~65 hours of focused work, LLM-assisted agent-team methodology (see "Methodology and transparency"). The 12-month **€30,000 grant** funds content-addressed persistence, TCP remoting + capability registry, kernel + device actors, CFPU integration demo, formal-spec groundwork (TLA+), and outreach. The applicant has **35+ years of professional software and hardware experience**: "Atlasz" MÁV railway dispatch with 26 years of uninterrupted live operation; AEE/QCassa Hungarian-tax-authority-certified cash-register software with 55+ Akka.NET actors; CLI-CPU reference simulator with 250+ tests. The project's **long-term vision** — the first engineering step toward the post-Dennard computing paradigm — is unfolded in the sections below.
...
- **Apache-2.0, fully libre:** Permissive license compatible with the broader .NET ecosystem — enterprise adoption path is clear. **The FenySoft Verified signing pipeline (CFPU AuthCode) is optional, not a prerequisite:** The Symphact runtime **works with full functionality** using a software `TCapabilityRegistry`; an organisation can deploy with its own signing infrastructure, an alternative signer trust-anchor, or without AuthCode at all. The FenySoft cert is an **optional hardware-level hot-load tampering protection** bought together with the CFPU silicon (see `docs/trust-model-en.md`), not a requirement for using the Symphact runtime. This is consistent with the AOSP model (Android Open Source Project is open; Google Play Services + the trademark are closed), but **Symphact itself is the full Apache-2.0 stack** — hardware-level enforcement is a HW purchase question, not a libre-runtime question. Under CLI-CPU RTL (CERN-OHL-S-2.0) anyone may manufacture a chip with their own trust anchor that is **not FenySoft Verified** but is fully Symphact-compatible.
...
**Methodology and transparency:** Development uses an **LLM-assisted TDD agent team** (Anthropic Claude Code: Architect + Implementer + Devil's Advocate + Test Guardian + HW Liaison roles), with the applicant providing the architectural decisions, code review, integration work, and human oversight of the TDD red-green-refactor loop. This explains the velocity documented in the roadmap (e.g. M0.4 estimated 24-32 hours, actual ~6 human-hours — `docs/roadmap-en.md:116`). The **€36/hour requested rate covers the human review, integration, and architectural decision-making hours**; the LLM API token cost is accounted for inside the `Online outreach + documentation` €2,500 line (~10-15% of the total budget). The 760-hour envelope is **achievable by a single human developer with LLM-assisted velocity over 12 months**, extrapolated from the past ~65 hours / 5 weeks pace (~13 hours/week). This methodology is **explicitly disclosed in `CONTRIBUTING.md` and the monthly research log**, in line with NLnet transparency expectations.
...
| **M5: Developer experience + docs + outreach** | NuGet package publication — **4 packages** matching the v0.5 maturity level: `Symphact.Core` (runtime core), `Symphact.Platform.DotNet` (.NET host platform), `Symphact.Persistence` (CAS journal + snapshot store), `Symphact.Remoting` (TCP transport + capability registry). `Symphact.Security` and `Symphact.Devices` are deliberately **not** separate packages at v0.5 — the capability registry is shared between Core / Remoting, and device actors stay inside Core or in samples. Further breakdown deferred to a later milestone (M1.x+) when there is a concrete reason. Plus `symphact` CLI tool, `symphact` CLI tool, bilingual architecture docs, contribution guide, **3 technical blog posts** (M1 persistence / M2 capability registry / M3-M4 unified primitive demo), **2-3 online technical demo videos** (for M1 / M2 / M3-M4 milestones, ~10-15 min each, YouTube or similar — travel-free, asynchronous, longer shelf life), public GitHub Pages docs + monthly research log. **Passive outreach target by Month 12:** 100+ GitHub stars, 5+ substantive issues / discussions from external participants, documented contact with ≥10 academic / industry touchpoints (contact list in the research log). **Active cohort-building is an INSTRUMENT, NOT A GOAL** in this phase: before the physical silicon demo (CLI-CPU F4 FPGA + F5 Tiny Tapeout, 2027–2028), industry / academic LoIs are **not realistic** — the actual cohort-building will be the **central deliverable of the follow-up grant**, once physically demonstrable HW exists. | ~180h | €6,500 | Ongoing | — (summary document + research log) |
| **M6: TLA+ specification of capability + supervision invariants — audit-input preparation** (roadmap M5.4 partial) | Initial formal specification of `send` / `receive` semantics in **TLA+** (chosen over Dafny: protocol-level spec, documented industrial precedent at AWS / Cosmos DB / Azure, closer to Symphact's messaging invariants than code-level refinement proof). Machine-checkable formalisation of capability model invariants, supervision lifecycle, and `TCapabilityRegistry` (TLC model checker runs). **Groundwork for a later (at maturer project stage) independent security audit** — at v0.5 startup stage external audit is premature; what happens here is only the preparation of audit-input evidence. Deliverable: ~3-5 TLA+ modules (Capability.tla, Supervision.tla, Send.tla), TLC model checker configuration, documented invariants. | ~80h | €2,900 | Month 9-12 | 0-1 issues (verification primitives) |
...
**Cost structure:**
- **Human engineering hours:** ~760 hours part-time × €36/hour ≈ €27,500 (architectural decisions, code review, integration, TDD red-green-refactor oversight — LLM-assisted methodology, see the "Methodology and transparency" section above; consistent with the parallel CLI-CPU proposal rate; ~63h/month part-time, in line with the M0.1–M0.4 actual velocity)
- **Online outreach + documentation + LLM API token cost (travel-free):** €2,500 — video production, GitHub Pages hosting, expanded technical docs, **plus the Anthropic Claude API token cost** of the agent-team development (estimated ~€1,500-2,000 over 12 months, remainder for video editing / audio gear / domain hosting)
...
**Schedule alongside the parallel CLI-CPU proposal and running FenySoft Kft. activities:**

| Period | CLI-CPU main focus | Symphact main focus | FenySoft Kft. ops | Total (hours/week) |
|--------|--------------------|---------------------|-------------------|--------------------|
| **2026 Q3 (months 1-3)** | F2 RTL bring-up (~40h/mo) | M1 persistence + M5 setup (~25h/mo) | Atlasz wind-down + QCassa ops (~35h/mo) | ~25 h/week |
| **2026 Q4 (months 4-6)** | F2 → F3 transition (~35h/mo) | M2 remoting + capability (~35h/mo) | Atlasz closure (2026-12-31) + QCassa ops (~30h/mo) | ~23 h/week |
| **2027 Q1 (months 7-9)** | F3 tape-out (~25h/mo) | M3 device actors deep work (~55h/mo) | QCassa/JokerQ ops (~20h/mo) | ~23 h/week |
| **2027 Q2 (months 10-12)** | F4 FPGA bring-up (~25h/mo) | M4 demo + M6 TLA+ + closing (~50h/mo) | QCassa/JokerQ ops (~20h/mo) | ~22 h/week |

**Average across the 12 months: ~23 hours/week combined for both grants + FenySoft Kft. ops.** This is **sustainable at a regular 30-35 hours/week with the LLM-assisted velocity** (a conservative 2× scaling from the past ~13h/week pace), and **the Symphact deliverables are weighted toward the 2027 calendar**, once the Atlasz contract (2026-12-31) has closed and CLI-CPU is in a quieter F3/F4 transition phase.
...
| **Anthropic Agent SDK / OpenAI Agents / LangGraph** | Multi-agent coordination framework in Python | Function-calling + JSON + JIT-eval; no capability-level isolation; ad-hoc audit | Capability-restricted actors, runtime + HW-level validation, supervision-based audit, EU AI Act-compliant tool-level audit substrate (not a certified system) |
...
- **Formal methods research community:** TLA+ specifications of `send`/`receive` semantics and capability invariants make Symphact a teaching and research target (see M6).
